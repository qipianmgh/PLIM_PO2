...
if(~(exist(gcspath,'dir')==7))
    ...
    methodName  = '@C843_GCS_Controller\\private\\LoadGCSDLL';
    replace     = '';
    ...
    if (matlabIs64bit)
        warning(['The default path of "C843_GCS2_DLL_x64.dll" could not be determined. Hence it is assumed "C843_GCS2_DLL_x64.dll" can be found in the folder of your sample: ', ...
            gcspath, ...
            '. If this folder doesn''t contain the dll, an error will occur.']);
    else
        warning(['The default path of "C843_GCS2_DLL.dll" could not be determined. Hence it is assumed "C843_GCS2_DLL.dll" can be found in the folder of your sample: ', ...
            gcspath, ...
            '. If this folder doesn''t contain the dll, an error will occur.']);
    end
...
if (~(exist(c.DLLName,'file') == 2))
    if (matlabIs64bit)
        error(['The folder of your sample ', ...
            '(', gcspath, ')', ...
            'doesn''t contain the "C843_GCS2_DLL_x64.dll". ', ...
            'This library is essential for communication between your Computer and the PI controller. ', ...
            'You can easily fix this problem by manually searching for the "C843_GCS2_DLL_x64.dll" on your system and copy it into the folder of your sample.']);
    else
        error(['The folder of your sample ', ...
            '(', gcspath, ')', ...
            'doesn''t contain the "C843_GCS2_DLL_x64.dll". ', ...
...
if(~libisloaded(c.libalias))
    % disp('Loading dll ...');
    disp('Loading dll for M-126 stages !');
    try
      % [notfound,warnings] = loadlibrary (c.DLLName,c.hfile,'alias',c.libalias);
        loadlibrary(c.DLLName, @C843_GCS_prototype,'alias',c.libalias);
    catch 
